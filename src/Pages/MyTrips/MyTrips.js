...
const MyTrips = () => {
    const { allBookings, handleDeleteBooking, fetchBookings, isDataLoadingFromApi } = useData();
    const { user } = useAuth();
    ...
    if (myTrips.length === 0) return (<React.Fragment>
        <NavBar></NavBar>
        <div className='w-full flex flex-col justify-center items-center h-96'>
            <p className="text-2xl">You don't have any bookings at this moment</p>
        </div>
        <Footer></Footer>
    </React.Fragment>);
